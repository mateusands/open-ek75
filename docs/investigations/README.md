...
| [`manual-fn-shortcuts.md`](manual-fn-shortcuts.md) | **New first-party source.** The retail unit's own printed manual, cross-checked against every decompiled Fn-shortcut finding above: mostly confirms them, corrects `key_input.py`'s wrong "US legends" claim outright (spec sheet says ABNT2), and turned up the actual brightness shortcut (§3.2, closed in `brightness-ladder.md`). Of its two flagged conflicts, `F9`/`F10`'s volume direction is now settled — tested live, the manual's own labels were swapped, `keymap.py`'s decode was right all along — and `Fn`+`Q`'s 2.4G-vs-Bluetooth label is still open. One untested lead remains: `Fn`+`[`/`Fn`+`]`, cycle lighting effect/colour — the first `LED_CMD_*` shortcut candidate that looks cleanly testable with `watch`. |
