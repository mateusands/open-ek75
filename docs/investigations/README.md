...
| [`brightness-ladder.md`](brightness-ladder.md) | **Ladder confirmed, use unconfirmed.** `{0, 70, 120, 190, 255}` is real and shaped like a step ladder, with clamp and wrap helpers — and nothing in the app calls any of them. A live attempt on real hardware (§3.1) neither confirmed nor ruled it out: `Fn`+arrows changed nothing, and `Fn`+`Space`'s one clean single-tap test also changed nothing — imprecise remote key timing, not a clean negative. Still open. |
